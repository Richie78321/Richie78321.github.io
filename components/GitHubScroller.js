...
export default function GitHubScroller() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const githubRepos = await (await fetch("https://api.github.com/users/Richie78321/events?per_page=50", {
        signal: abortController.signal,
      })).json();

      githubRepos.filter((event) => event.type === "PushEvent");
      setRecentCommits(githubRepos.reduce((acc, pushEvent) => {
        if (pushEvent.repo && pushEvent.payload?.commits) {
          acc.push(...pushEvent.payload.commits.map((commit) => `Committed to ${pushEvent.repo.name} ${moment(pushEvent.created_at).fromNow()}: ${commit.message}`));
        }
  ...
  return (
    <div class="mx-4">
      <hr />
      I'm always doing something:<br />
      <div className="text-truncate my-1 text-primary"><TextScroller textToScroll={recentCommits} /></div>
